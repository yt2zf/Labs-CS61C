...
Student* create_student_1(int id) {
  // allocate memory to store a Student struct
  Student *student_ptr = malloc(sizeof(Student));

  // set student_ptr's id to the id argument of this function
  student_ptr->id = id;
...
void create_student_2(Student** student_double_ptr, int id) {
  // fill the space that student_double_ptr points to with the address of
  //       some memory large enough for a Student struct
  // you may need to use the dereference operator here
  *student_double_ptr = malloc(sizeof(Student));

  // set student_double_ptr's id to the id argument of this function
  (*student_double_ptr)->id = id;
}


int main() {
  // use create_student_1 to create a pointer to a Student struct
  //       where the student has id of 5
  Student *student1_ptr = create_student_1(5);

  // print the id of the student that student1_ptr points to
  printf("Student 1's ID: %d\n", student1_ptr->id);

  // create a pointer that can point to a Student struct
  //       do not allocate any memory
  Student *student2_ptr;

  // use create_student_2 to populate the student2_ptr
  ...
  // print the id of the student that student2_ptr points to
